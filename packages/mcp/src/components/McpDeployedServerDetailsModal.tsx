...
export const McpDeployedServerDetailsModal: React.FC<McpDeployedServerDetailsModalProps> = ({
  ...
  const renderSpecTab = () => (
    <DescriptionList isHorizontal columnModifier={{ default: '2Col' }}>
      ...
      {server.spec.config && (
        <>
          <DescriptionListGroup>
            ...
            <DescriptionListDescription>
              {server.spec.config.command ? (
                <code className="pf-v6-u-font-family-monospace pf-v6-u-font-size-sm">
                  {server.spec.config.command.join(' ')}
                </code>
              ) : (
                '—'
              )}
          ...
          <DescriptionListGroup>
            ...
            <DescriptionListDescription>
              {server.spec.config.args ? (
                <code className="pf-v6-u-font-family-monospace pf-v6-u-font-size-sm">
                  {server.spec.config.args.join(' ')}
                </code>
              ) : (
                '—'
              )}
          ...
          {server.spec.config.env && server.spec.config.env.length > 0 && (
            <DescriptionListGroup>
              <DescriptionListTerm>Environment Variables</DescriptionListTerm>
              <DescriptionListDescription>
                {server.spec.config.env.map((envVar) => (
                  <div key={envVar.name}>
                    <code className="pf-v6-u-font-family-monospace pf-v6-u-font-size-sm">
      ...
      {server.spec.deployment && (
        <>
          <DescriptionListGroup>
            <DescriptionListTerm>Replicas</DescriptionListTerm>
            <DescriptionListDescription>
              {server.spec.deployment.replicas || 1}
            </DescriptionListDescription>
          </DescriptionListGroup>

          {server.spec.deployment.resources && (
            <DescriptionListGroup>
              <DescriptionListTerm>Resources</DescriptionListTerm>
              <DescriptionListDescription>
                {server.spec.deployment.resources.requests && (
                  <div>
                    <strong>Requests:</strong> CPU:{' '}
                    {server.spec.deployment.resources.requests.cpu || '—'}, Memory:{' '}
                    {server.spec.deployment.resources.requests.memory || '—'}
                  </div>
                )}
                {server.spec.deployment.resources.limits && (
                  <div>
                    <strong>Limits:</strong> CPU:{' '}
                    {server.spec.deployment.resources.limits.cpu || '—'}, Memory:{' '}
                    {server.spec.deployment.resources.limits.memory || '—'}
                  </div>
                )}
              </DescriptionListDescription>
            </DescriptionListGroup>
